*! 0.1.0 Gustavo Iglésias 14dec2018
...
// csv_file: csv file with logical conditions
// id: id for observations. when the user doesn´t specify an id, one is created inside the program (_n)
// out_path: if not specified, folders and files (.dta, .html, .smcl and .stmd) will be saved in the current working directory
// linesize sets line size in stata. The default value is 255
...
// will not be saved
// save_obs specifies the number of inconsistencies to be saved. The default value is 50. This option is only allowed when option internal is active
// mpz creates a matrix with missing values, positive values and zeros and observations for which value labels are missing. If absent, the default matrix will contain all 
